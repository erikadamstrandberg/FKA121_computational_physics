...
void fft_forward(double *signal, double *transform, int n) /* input data, output powspec_data, number of timesteps */
{
	/*make copy of data to avoid messing with data in the transform*/
	for (int i = 0; i < n; i++)	{
		transform[i] = signal[i];
	}
	...
	/*Do the fft*/
	gsl_fft_real_transform(transform, 1, n, real, work);	
...
/* Makes fft of data and returns the powerspectrum in powspec_data */
void fft_inverse(double *transform, double *signal, int n) /* input data, output powspec_data, number of timesteps */
{   
    /* Declaration of variables */
    ...
    /*Do the fft*/
    gsl_fft_halfcomplex_inverse(signal, 1, n, hc, work);
